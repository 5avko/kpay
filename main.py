...
class UserState(StatesGroup):
    ...
    ADMIN_GRANTING_FREE_SUBSCRIPTION = State()
    ...
    async with aiosqlite.connect('subscriptions.db') as db:
        async with db.execute('SELECT * FROM subscriptions') as cursor:
            async for row in cursor:
                user_id, expiration_date_str = row
...
async def save_subscriptions():
    async with aiosqlite.connect('subscriptions.db') as db:
        await db.executemany(
            ...
            [(user_id, expiration_date.isoformat()) for user_id, expiration_date in user_subscriptions.items()]
        )
        await db.commit()
    ...
    for user_id, expiration_date in list(user_subscriptions.items()):
        try:
            ...
            if chat_member.user.is_bot:
                logger.warning(f"Skipping subscription check for bot user: {user_id}")
                continue
            ...
            if expiration_date < current_time:
                ...
                await bot.send_message(user_id, f"Ваша подписка истекает через {(expiration_date - current_time).days} дней. Не забудьте продлить ее. Нажмите /start , чтобы возобновить работу бота.")
            ...
            logger.error(f"Chat not found for user: {user_id}. Removing from subscriptions.")
            del user_subscriptions[user_id]
            await save_subscriptions()
        except aiogram_exceptions.Unauthorized:
            logger.error(f"Unauthorized error for user: {user_id}. Removing from subscriptions.")
            del user_subscriptions[user_id]
            await save_subscriptions()
        except Exception as e:
...
async def show_subscription_options(message: types.Message):
    ...
    keyboard.add(InlineKeyboardButton("3 дня - 2$", callback_data="sub:3:days:2"),
                 InlineKeyboardButton("5 дней - 3$", callback_data="sub:5:days:3"),
                 InlineKeyboardButton("7 дней - 4$", callback_data="sub:7:days:4"))
...
async def show_currency_options(message: types.Message, price: float, subscription_option: str):
    ...
    for currency in currencies:
        ...
        keyboard.insert(InlineKeyboardButton(
            ...
            callback_data=f"pay:{subscription_option}:{formatted_price}:{currency}"
        ))
...
async def update_user_rank(user_id):
    ...
    deal_count = sheet.max_row - 1  # Исключаем заголовок
    return get_user_rank(deal_count)
...
async def process_successful_payment(user_id, amount, subscription_option):
    ...
    await bot.send_message(user_id, f"Подписка активирована до {expiration_date.strftime('%d.%m.%Y %H:%M')}")
...
async def account_info(message: types.Message, state: FSMContext, **kwargs):
    ...
    logger.info(f"User {message.from_user.id} checking account. Current state: {current_state}")
    file_path = os.path.join(USER_FILES_DIR, f'user_{user_id}.xlsx')
    
    if not os.path.exists(file_path):
        await message.reply("У вас еще нет данных о сделках.")
        return

    workbook = openpyxl.load_workbook(file_path)
    sheet = workbook.active

    first_deal_date_value = sheet['A2'].value if sheet.max_row > 1 else datetime.datetime.now()
    
    if isinstance(first_deal_date_value, str):
        try:
            first_deal_date = datetime.datetime.strptime(first_deal_date_value, '%d.%m.%Y')
        except ValueError:
            first_deal_date = datetime.datetime.now()
    else:
        first_deal_date = first_deal_date_value

    total_deals = sheet.max_row - 1  # Исключаем заголовок
    total_buys = 0
    total_sells = 0
    ...
    user_rank = await update_user_rank(user_id)

    if user_id in user_subscriptions:
        expiration_date = user_subscriptions[user_id]
        time_left = expiration_date - datetime.datetime.now()
        days_left = time_left.days
        hours_left = time_left.seconds // 3600
        subscription_info = f"Ваша подписка действительна до: {expiration_date.strftime('%d.%m.%Y %H:%M')}\n" \
                            f"Осталось: {days_left} дней и {hours_left} часов\n\n"
    else:
        subscription_info = "У вас нет активной подписки.\n\n"

    stats = f"{subscription_info}" \
            f"Статистика:\n" \
            f"Ваш ранг: {user_rank}\n" \
            f"Участник нашей системы с: {first_deal_date.strftime('%d.%m.%Y')}\n" \
            f"Количество действий в нашей системе: {total_deals}\n" \
            f"Сумма покупок: {total_buys:.2f} RUB\n" \
            f"Сумма продаж: {total_sells:.2f} RUB\n" \
            f"Прибыль: {profit:.2f} RUB"

    await message.reply(stats)

    # Отправка изображения ранга
    rank_image_path = RANK_IMAGES.get(user_rank)
    if rank_image_path and os.path.exists(rank_image_path):
        with open(rank_image_path, 'rb') as photo:
            await bot.send_photo(message.chat.id, photo, caption=f"Ваш ранг: {user_rank}")
    else:
        logger.error(f"Ранг {user_rank} не имеет связанного изображения или путь не существует.")
        

  

# Добавим логирование в start_day и на переход состояния
...
async def start_day(message: types.Message, state: FSMContext, **kwargs):
    user_id = message.from_user.id
    current_state = await state.get_state()
    logger.info(f"User {user_id} attempting to start day. Current state: {current_state}")
    file_path = os.path.join(USER_FILES_DIR, f'user_{user_id}.xlsx')
    
    logger.info(f"User {user_id} is attempting to start the workday.")
    ...
    logger.info(f"User {user_id} successfully started the workday.")
    ...
    logger.info(f"User {user_id} state set to WORKING_DAY_STARTED.")
...
async def end_day(message: types.Message, state: FSMContext, **kwargs):
    ...
    logger.info(f"User {message.from_user.id} state set to WORKING_DAY_NOT_STARTED")
...
@dp.message_handler(Text(equals="Начать рабочий день"), state='*')
async def handle_unexpected_start_day(message: types.Message, state: FSMContext):
    current_state = await state.get_state()
    logger.warning(f"User {message.from_user.id} tried to start day in unexpected state: {current_state}")
    await message.reply("Пожалуйста, сначала завершите текущий рабочий день.")
