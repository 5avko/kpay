...
logger.info(f"aiocryptopay version:")
...
class UserState(StatesGroup):
    ...
    ADMIN_GRANTING_FREE_SUBSCRIPTION = State()  # Новое состояние
...
async def download_table(message: types.Message, state: FSMContext, **kwargs):
    ...
    if os.path.exists(file_path):
        with open(file_path, 'rb') as file:
            ...
            await message.answer("Таблица скачана.", reply_markup=account_button())
    else:
